...
class Handler():
    def __init__(self, trigger_checker: BaseTrigger, responder: BaseResponse, probability_to_respond: float = 1):
        self.trigger_checker = trigger_checker
        self.responder = responder

        if probability_to_respond > 1 or probability_to_respond <= 0:
            raise ValueError(f'Parameter \'probability_to_respond\' should be in range (0, 1], '
                             'but {probability_to_respond} was given')

        self.probability_to_respond = probability_to_respond
