...
class RulesParsingHelper:
    ...
    @staticmethod
    def add_globals(config: CommentedMap, params: TriggerResponseParams):
        if params.plural_key in config:
            params.globals.update(params.parser.parse_many(config[params.plural_key]))

    @staticmethod
    def evaluate_boolean_expression(expr: boolean.Expression, params: TriggerResponseParams) -> TriggerResponse:
    ...
    @staticmethod
    def parse_rule_trigger_or_response(rule: CommentedMap, params: TriggerResponseParams) -> TriggerResponse:
